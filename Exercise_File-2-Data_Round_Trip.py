...
#    Python, so we first need to import its definition from the saspy module.
#
# 3. The code from saspy import SASsession; sas = SASsession() only needs to be
#    used to establish a SAS kernel connection once within a given Python
#    session. All subsequent cells in this Notebook will assume these lines
#    have been executed.
...
#    * summary information about its 6 numerical columns, which is obtained by
#      fish_df calling its describe method (the pandas equivalent of the SAS
#      MEANS procedure)
#
...
#    to access the SAS library sashelp defined within this SAS session and to
#    load the entire contents of SAS dataset sashelp.fish into the DataFrame
#    fish_df.
#
...
# Notes:
#
# 1. The DataFrame fish_df, which was created in a cell above from the SAS
...
#      method to aggregate by the functions in the
#      list ['count', 'std', 'mean', 'min', 'max'].
#
# 3. Identical results could be obtained using the following SAS code:
#        proc means data=sashelp.fish std mean m
#             var Weight;
#        run;
#    However, while PROC MEANS operates on SAS datasets row-by-row from disk,
#    DataFrames are stored entirely in main in max;
# #             class species;memory. This allows any number of
...
# Notes:
#
# 1. The DataFrame fish_df_gsa, which was created in a cell above from the SAS
#    dataset sashelp.fish, is used to create the new SAS dataset
