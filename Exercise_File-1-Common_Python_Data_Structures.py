...
#    key-value pairs is created, and the
#    following are printed:
...
#    called _associative arrays_ or _maps_ because they map keys (appearing
#    before the colons) to values (appearing after the colons). In other words,
#    the value associated with each key can
...
#
# 4. In Python 3.5, the print order of key-value pairs may not match insertion
#    order, meaning the order key-value pairs are listed when the dictionary is
#    created. As of Python 3.7 (released in June 2018), insertion order is
#    preserved.
#
...
#    (2 rows by 3 columns) is created, and the following are printed with blank
#    lines between them:
...
#      its info method (meaning a function whose definition is nested inside it)
#
# 2. Since DataFrames are not built into Python, we must first import their
#    definition from the pandas module. Like their R counterpart, DataFrames
#    are two-dimensional arrays of values that can be thought of like SAS
#    datasets. However, while SAS datasets are typically only accessed from
#    disk and processed row-by-row, DataFrames are loaded into memory all at
#    once. This means values in DataFrames can be randomly accessed, but it
#    also means the size of DataFrames can't grow beyond available memory.
#
