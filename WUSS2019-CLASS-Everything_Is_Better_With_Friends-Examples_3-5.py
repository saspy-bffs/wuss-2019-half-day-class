...
#
# Examples 3-5 Common Python Data Structures

# Lines 12 through 18 load necessary modules and define a function to format output.
...
# Notes:
# 1. A list object named hello_world_list with two values is created, and the following are printed:
#   * the value of the list
#   * its type (which is <class 'list'>)
#
# 2. Lists are the most fundamental Python data structure and are related to SAS data-step arrays. Values in lists are
#    always kept in insertion order, meaning the order they appear in the list's definition, and they can be
...
#   * The left-most element of a list is always at index 0. Unlike SAS, customized indexing is only available for more
#     sophisticated data structures in Python (e.g., a dictionary, as in the next example).
#
...
# Notes:
# 1. A dictionary (dict for short) object named hello_world_dict with three key-value pairs is created, and the
#    following are printed:
#   * the value of the dictionary
#   * its type (which is <class 'dict'>)
#
# 2. Dictionaries are another fundamental Python data structure and are related to SAS formats and data-step hash
#    tables. Dictionaries are more generally called _associative arrays_ or _maps_ because they map keys (appearing
#    before the colons) to values (appearing after the colons). In other words, the value associated with each key can
#    be accessed using bracket notation:
...
#
# 4. In Python 3.5, the print order of key-value pairs may not match insertion order, meaning the order key-value pairs
#    are listed when the dictionary is created. As of Python 3.7 (released in June 2018), insertion order is preserved.
#
...
# Notes:
#
# 1. A DataFrame (df for short) object named hello_world_df with dimensions 2x3 (2 rows by 3 columns) is created, and
...
#   * some information about it, which is obtained by hello_world_df calling its info method (meaning a function whose
#     definition is nested inside it)
#
# 2. Since DataFrames are not built into Python, we must first import their definition from the pandas module. Like
#    their R counterpart, DataFrames are two-dimensional arrays of values that can be thought of like SAS datasets.
#    However, while SAS datasets are typically only accessed from disk and processed row-by-row, DataFrames are loaded
#    into memory all at once. This means values in DataFrames can be randomly accessed, but it also means the size of
#    DataFrames can't grow beyond available memory.
#
# 3. The dimensions of the DataFrame are determined as follows:
#   * The keys 'salutation', 'valediction', and 'part of speech' of the dictionary passed to the DataFrame constructor
#     function become column labels.
#   * Because each key maps to a list of length two, each column will be two elements tall (with an error occurring if
#     the lists are not of non-uniform length).
#
# 4. This example gives one option for building a DataFrame, but the constructor function can also accept many other
#    object types, including another DataFrame.
#
# 5. For extra credit, try any or all of the following (keeping in mind that DataFrames can be indexed like
#    dictionaries):
